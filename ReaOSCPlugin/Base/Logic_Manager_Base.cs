...
namespace Loupedeck.ReaOSCPlugin.Base
{
    ...
    public class Logic_Manager_Base : IDisposable
    {
        ...
        #region 模式管理
        public void RegisterModeGroup(ButtonConfig config) { var modeName = config.DisplayName; var modes = config.Modes; if (string.IsNullOrEmpty(modeName) || modes == null || modes.Count == 0) return; if (!_modeOptions.ContainsKey(modeName)) { _modeOptions[modeName] = modes; _currentModes[modeName] = 0; _modeChangedEvents[modeName] = null; PluginLog.Info($"[LogicManager] 模式组 '{modeName}' 已注册，包含模式: {string.Join(", ", modes)}"); } }
        public void ToggleMode(string modeName) { if (_currentModes.TryGetValue(modeName, out _) && _modeOptions.TryGetValue(modeName, out var options)) { _currentModes[modeName] = (_currentModes[modeName] + 1) % options.Count; _modeChangedEvents[modeName]?.Invoke(); PluginLog.Info($"[LogicManager] 模式组 '{modeName}' 已切换到: {GetCurrentModeString(modeName)}"); } }
            ...
            foreach (var config in configs)
            {
                ...
                if (config.ActionType == "ToggleButton" || config.ActionType == "ToggleDial")
                {
                    ...
                    if (!string.IsNullOrEmpty(effectiveOscAddressForStateListener) && effectiveOscAddressForStateListener != "/")
                    {
                        ...
                        PluginLog.Info($"[LogicManager] Action '{actionParameter}' (Type: {config.ActionType}) 初始状态从 OSC Addr '{effectiveOscAddressForStateListener}' 获取为: {this._toggleStates.GetValueOrDefault(actionParameter)}");

                        if (config.ActionType == "ToggleDial")
                        {
                            this._oscAddressToActionParameterMap[effectiveOscAddressForStateListener] = actionParameter;
                            PluginLog.Info($"[LogicManager] 映射ToggleDial: OSC '{effectiveOscAddressForStateListener}' -> ActionParam '{actionParameter}'");
                        }
                    }
                    else
                    {
                        this._toggleStates[actionParameter] = false;
                        PluginLog.Warning($"[LogicManager] Action '{actionParameter}' (Type: {config.ActionType}) 无法确定有效OSC监听地址，初始状态设为 false。");
                    }
                }
        ...
        private void OnOSCStateChanged(object sender, OSCStateManager.StateChangedEventArgs e)
        {
            ...
            {
                if (this._allConfigs.TryGetValue(mappedActionParameter, out var config) && config.ActionType == "ToggleDial")
                {
                    ...
                    if (!this._toggleStates.ContainsKey(mappedActionParameter) || this._toggleStates[mappedActionParameter] != newState)
                    {
                        ...
                        PluginLog.Info($"[LogicManager] ToggleDial状态更新 (通过映射): ActionParam '{mappedActionParameter}' (来自OSC '{e.Address}') = {newState}");
        ...
        public void ProcessDialAdjustment(ButtonConfig config, int ticks, string actionParameter, Dictionary<string, DateTime> lastEventTimes)
        {
            ...
            if (config.ActionType == "ToggleDial")
            {
                ...
                {
                    basePathSuffix = displayNameForPath;
                }
                string toggleDialAddress = $"/{groupNameForPath}/{basePathSuffix}";
                toggleDialAddress = toggleDialAddress.Replace("//", "/");

                if (string.IsNullOrEmpty(toggleDialAddress) || toggleDialAddress == "/")
                {
                    PluginLog.Warning($"[LogicManager] ToggleDial '{actionParameter}' 生成的 OSC 地址无效。");
                    return;
                }

                float valueToSend = (ticks > 0) ? 1.0f : 0.0f;
                ReaOSCPlugin.SendOSCMessage(toggleDialAddress, valueToSend);
                PluginLog.Info($"[LogicManager] ToggleDial '{actionParameter}' SENT to '{toggleDialAddress}' value {valueToSend}");
                return;
            }

            string jsonIncreaseAddress;
            string jsonDecreaseAddress;
            ...
            string finalIncreaseOscAddress;
            string finalDecreaseOscAddress;
            ...
            if (!string.IsNullOrEmpty(targetAddress) && targetAddress != "/")
            {
                ...
                PluginLog.Info($"[LogicManager] Dial '{actionParameter}' SENT to '{targetAddress}' value {Math.Abs(ticks) * acceleration}");
            }
            else
            {
                PluginLog.Warning($"[LogicManager] Dial '{actionParameter}' 最终计算的目标 OSC 地址无效: '{targetAddress}'");
            }
        }
        ...
        public bool ProcessDialPress(ButtonConfig config, string actionParameter)
        {
            ...
            if (!string.IsNullOrEmpty(config.ResetOscAddress))
            {
                ...
                { PluginLog.Warning($"[LogicManager] Dial Reset '{actionParameter}' (ResetAddress: '{config.ResetOscAddress}') 生成的 OSC 地址无效。"); }
                else
        ...
        public void Dispose()
        {
            ...
            {
                foreach (Delegate d in this.CommandStateNeedsRefresh.GetInvocationList())
                {
                    this.CommandStateNeedsRefresh -= (EventHandler<string>)d;
                }
            }
        }
        #endregion
